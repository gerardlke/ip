...
public class DialogBox extends HBox {
    ...
    private static final Image FINN_IMAGE = new Image(
            DialogBox.class.getResource("/images/DaFinn.png").toExternalForm());
     ...
     * @param user Whether the message belongs to the user rather than Finn.
    ...
    private DialogBox(String text, boolean user, boolean error) {
        ...
        if (user) {
            ...
            getChildren().add(0, createFinnAvatar());
        }
    ...
    /**
     * Creates Finn's avatar using the same size and circular crop as the user's avatar.
     *
     ...
     *
     * @param text Finn's response text.
     * @param error Whether the response reports a failed command.
     * @return The message row, ready to add to the conversation on the JavaFX thread.
     */
    public static DialogBox getFinnDialog(String text, boolean error) {
